...
class RNN(nn.Module):
    def __init__(self, input_shape, hidden_dim):
        ...
        self.rnn = nn.GRU(self.input_shape, self.hidden_dim, num_layers=self.num_layers, batch_first=True, dropout=0.5) #
        # self.rnn = nn.LSTM(self.input_shape, self.hidden_dim, num_layers=self.num_layers, batch_first=True, dropout=0.5) #
        # self.fc2 = nn.Linear(args.rnn_hidden_dim, args.n_actions)

    def forward(self, obs):
        hidden_state = nn.init.xavier_uniform_(th.Tensor(self.num_layers, obs.shape[0], self.hidden_dim), gain=1)
        cell_state = nn.init.xavier_uniform_(th.Tensor(self.num_layers, obs.shape[0], self.hidden_dim), gain=1)
        ...
        # output, (h1,c1) = self.rnn(obs, (hidden_state.cuda(),cell_state.cuda()))
        # output_shape = output.shape
        # output = f.relu(output.reshape(output_shape))
        return output
