...
class MCTS_BufferSamples(NamedTuple):
    observations: th.Tensor
    actions: th.Tensor
    policies: th.Tensor
    dones: th.Tensor
...
class ReplayBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        self.action_dim = selected_num    # replace the action_dim with selected_num
    ...
    def sample(self, batch_size: int, env: Optional[GymEnv] = None) -> ReplayBufferSamples:
        """
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[GymEnv] = None) -> ReplayBufferSamples:
...
class MCTS_ReplayBuffer(BaseBuffer):
    """
    Replay buffer used in off-policy algorithms like SAC/TD3.
    :param buffer_size: Max number of element in the buffer
    :param observation_space: Observation space
    ...
    def __init__(
        ...
        # self.observations = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=observation_space.dtype)
        self.observations = np.zeros((self.buffer_size, self.n_envs, self.obs_shape[0], self.action_space.n+1), dtype=self.observation_space.dtype)
        ...
        # else:
        #     self.next_observations = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=observation_space.dtype)
        self.policies = np.zeros((self.buffer_size, self.n_envs, self.action_space.n), dtype=np.float32)

        # self.actions = np.zeros((self.buffer_size, self.n_envs, self.action_dim), dtype=self.action_space.dtype)
        self.actions = np.zeros((self.buffer_size, self.n_envs, 1), dtype=np.int32)
    ...
    def add(
        ...
        # Copy to avoid modification by reference
        self.observations[self.pos] = np.array(obs).copy()
        self.policies[self.pos] = np.array(policy).copy()
        # if self.optimize_memory_usage:
        #     self.observations[(self.pos + 1) % self.buffer_size] = np.array(next_obs).copy()
        # else:
        #     self.next_observations[self.pos] = np.array(next_obs).copy()
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[GymEnv] = None) -> ReplayBufferSamples:
        ...
        data = (
            ...
            self.actions[batch_inds, env_indices, :],
            # next_obs,
            self.policies[batch_inds, env_indices, :],
            # Only use dones that are not due to timeouts
            # deactivated by default (timeouts is initialized as an array of False)
        ...
        return MCTS_BufferSamples(*tuple(map(self.to_torch, data)))
    ...
    def reset(self) -> None:
        ...
        self.rewards = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.dones = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        super(MCTS_ReplayBuffer, self).reset()