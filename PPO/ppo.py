...
from stable_baselines3.common.utils import explained_variance, set_random_seed, get_device, update_learning_rate, get_schedule_fn, get_linear_fn, obs_as_tensor, safe_mean
...
class PPO():
    ...
    def __init__(
        ...
        clip_range_vf: Union[None, float, Schedule] = 0.1,
    ...
    def train(self, rollout_buffer, progress_remaining) -> None:
        ...
        self._update_learning_rate(self.policy.optimizer, progress_remaining)
        ...
        entropy_losses = []
        pg_losses, value_losses, losses = [], [], []
        clip_fractions = []
        # if self.num_timesteps in [825,960,1650,1785,2055]:
        #     ipdb.set_trace()
        ...
        for epoch in range(self.n_epochs):
            ...
            self.ppo_n_updates += 1

            if not continue_training:
                break
        # print(np.mean(pg_losses),self.policy.value_net.weight[:2,:10])
        # wandb.log(
        #     {"learn rate": self.policy.optimizer.param_groups[0]['lr'],
        #     "ppo_policy_loss": np.mean(pg_losses),
        #     "ppo_value_loss": np.mean(value_losses),
        #     "ppo_final_loss": np.mean(losses)},
        #     # step=self.ppo_n_updates
        #     ) 
        # explained_var = explained_variance(self.rollout_buffer.values.flatten(), self.rollout_buffer.returns.flatten())

        # file_path = '/home/zetao/HRL-idxtrack/results/indtrack1/indtrack2.xlsx'  # 替换为你的文件路径
        # for roll_data in rollout_buffer.get(1024):
        #     new_data = [
        #         {
        #         'n_update': self._n_updates,
        #         'pg_losses_mean': np.mean(pg_losses), 
        #         'n_pg_loss':len(pg_losses),
        #         'pg_losses': pg_losses,
        #         'values_losses_mean': np.mean(value_losses),
        #         'ratio': ratio,
        #         # 'rewards': roll_data.rewards,
        #         'advantages': roll_data.advantages,
        #         'returns': roll_data.returns,
        #         # 'values': roll_data.values,
        #         # 'starts': roll_data.episode_starts
        #         }  # 替换为你的数据       
        #     ]
        # if not os.path.exists(file_path):
        #     df = pd.DataFrame(new_data, columns=['n_update','pg_losses_mean','n_pg_loss','pg_losses','values_losses_mean','ratio','rewards','advantages','returns','starts'])  # 替换为你的列名
        #     df.to_excel(file_path, index=False)
        # else:
        #     # 如果文件已存在，读取现有的Excel文件并追加数据
    ...
    def _update_learning_rate(self, optimizers: Union[List[th.optim.Optimizer], th.optim.Optimizer], progress_remaining) -> None:
        """
        ...
        for optimizer in optimizers:
            update_learning_rate(optimizer, self.lr_schedule(progress_remaining))
